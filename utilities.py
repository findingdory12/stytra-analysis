...
class Experiment(dict):
    ...
    log_mapping = dict(
        stimulus_param_log=["dynamic_log",
                            "stimulus_log", "stimulus_param_log"],
    ...
    def _get_log(self, log_name):
        ...
        if getattr(self, uname) is None:
            for possible_name in self.log_mapping[log_name]:
                try:
                    logname = next(
                        self.path.glob(self.session_id + "_" +
                                       possible_name + ".*")
                    ).name
    ...
    def _load_log(self, data_name):
        ...
        if file.suffix == ".csv":
            ...
            raise ValueError(
                str(data_name) +
                " format is not supported, trying to load " + str(file)
            )

    def stimulus_starts_ends(self):
        starts = np.array([stim["t_start"]
                          for stim in self["stimulus"]["log"]])
...
def extract_bouts(
    ...
    **kwargs
    ...
    for i_fish in range(n_fish):
        ...
        vel = dfint["f{}_vx".format(i_fish)] ** 2 + \
...
def x_y_stackbinning(stack, factor):
    ...
    binned = trimmed.reshape(
        trm_dims[:-1] + (int(trm_dims[-1] / factor), factor)).mean(-1)
    binned = (
        binned.swapaxes(-1, -2)
        .reshape(
            trm_dims[:-2]
